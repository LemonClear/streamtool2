...
{
        int ret = -1;
        char *regconfig = "./configs/regconfig.core";
        char delimit = 0;
...
int core_init(ip *father, ip *core, int id, param *params)
{
        ...
        for (sub = 0; sub < params->core_reg_count; sub++) {
                ...
                /*table addr2reg*/
                addr2str = hexdui2s(core->reglist[sub]->address);
        ...
        for (sub = 0; sub < (params->ncp_count + params->tcp_count +
                ...
                /*table addr2subip*/
                addr2str = hexdui2s(core->subips[sub]->address);
